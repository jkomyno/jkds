...
namespace {
  ...
  class MaxKHeapTest : public ::testing::Test {
  protected:
    MaxKHeapTest() {
...
TEST_F(MaxKHeapTest, empty) {
  auto empty_max_heap = make_max_k_heap<4, false, uint8_t>();
...
TEST_F(MaxKHeapTest, empty_pod) {
  auto empty_max_heap_pod = make_max_k_heap<4, false, shared_ptr<Pod>>();
...
TEST_F(MaxKHeapTest, full) {
  auto max_heap = make_max_k_heap<4>(test_vector);
...
TEST_F(MaxKHeapTest, already_heap) {
  auto already_max_heap = make_max_k_heap<4, true>(already_max_seed);
...
TEST_F(MaxKHeapTest, init_copy) {
  std::vector<uint8_t> vector{0, 1, 2, 3};
  auto heap = make_max_k_heap<4>(vector);
  ASSERT_EQ(heap.size(), 4);
  ASSERT_EQ(vector.size(), 4);
}

TEST_F(MaxKHeapTest, init_move) {
  std::vector<uint8_t> vector{0, 1, 2, 3};
  auto heap = make_max_k_heap<4>(std::move(vector));
  ASSERT_EQ(heap.size(), 4);
