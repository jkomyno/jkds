...
TEST_F(MinBinaryHeapTest, empty) {
  auto empty_max_heap = make_min_heap<false, uint8_t>();
...
TEST_F(MinBinaryHeapTest, empty_pod) {
  auto empty_max_heap_pod = make_min_heap<false, shared_ptr<Pod>>();
...
TEST_F(MinBinaryHeapTest, full) {
  auto max_heap = make_min_heap(test_vector);
...
TEST_F(MinBinaryHeapTest, already_heap) {
  auto already_max_heap = make_min_heap<true>(already_max_seed);
...
TEST_F(MinBinaryHeapTest, init_copy) {
  std::vector<uint8_t> vector{0, 1, 2, 3};
  auto heap = make_min_heap(vector);
  ASSERT_EQ(heap.size(), 4);
  ASSERT_EQ(vector.size(), 4);
}

TEST_F(MinBinaryHeapTest, init_move) {
  std::vector<uint8_t> vector{0, 1, 2, 3};
  auto heap = make_min_heap(std::move(vector));
  ASSERT_EQ(heap.size(), 4);
